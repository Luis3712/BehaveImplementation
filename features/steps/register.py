...
def step_impl(context):
    context.register_page.enter_email("testvalidation1234@outlook.com")


@then(u'Proper warning message informing about duplicate account should be displayed')
def step_impl(context):
    assert context.register_page.display_status_of_duplicate_email_waring_message("Warning: E-Mail Address is already registered!")
...
def step_impl(context):
    expected_privacy_policy_waring = "Warning: You must agree to the Privacy Policy!"
    ...
    context.register_page.display_status_of_all_warnings_messages(expected_privacy_policy_waring, expected_first_name_warning, expected_last_name_warning,
